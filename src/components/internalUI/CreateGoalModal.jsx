...
import { Target, Check, X, Loader2, ShieldCheck, Flag, ChevronLeft,
  Plane, Laptop, Car, GraduationCap, Home, Shield, Heart, Briefcase, Gift, Sparkles } from 'lucide-react'
...
function calcPeriods(startDate, maturityDate, frequency) {
...
export default function CreateGoalModal({ product, onClose, onSubmit, submitting, success }) {
  ...
  const [autoContrib] = useState(true)
  ...
  function submit() {
    ...
    onSubmit({
      ...
      contribution: autoContrib ? numPreferred : null,
      frequency: autoContrib ? frequency : null,
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-[9.5px] uppercase tracking-[1.3px] text-brand-accent font-bold m-0">
                  {success ? 'Goal created' : `Step ${step} of 2`}
                </p>
            ...
            {success ? (
                ...
                >
                  {[
                    ...
                    >
                      ...
                      <span className="text-[12.5px] font-bold text-[var(--c-text)] tabular-nums capitalize">{r.value}</span>
                    </div>
                  ))}
                </div>
              ...
              >
                ...
                <label className="flex flex-col gap-1.5">
                  ...
                  <input
                    type="text" value={name} onChange={e => { const v = e.target.value; setName(v.length ? v[0].toUpperCase() + v.slice(1) : v) }}
                ...
                <div className="flex flex-col gap-2">
                  <span className="text-[10.5px] font-bold uppercase tracking-[1px] text-[var(--c-text-muted)]">Target category</span>
                    ...
                    {CATEGORIES.map(({ id, label, Icon }) => {
                      ...
                      return (
                        <button
                          ...
                          className="flex flex-col items-center justify-center gap-1.5 py-2.5 px-1 rounded-xl border transition text-center"
                          style={active
              ...
              >
                ...
                >
                  <div>
                    <p className="text-[10px] uppercase tracking-[1px] font-bold m-0" style={{ color: TEAL }}>{category}</p>
                    <p className="text-[13px] font-black text-[var(--c-text)] m-0 mt-0.5 truncate max-w-[160px]">{name}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] text-[var(--c-text-muted)] m-0">Target</p>
                    <p className="text-[13px] font-black text-[var(--c-text)] tabular-nums m-0">{fmtN(numGoal)}</p>
                  </div>
                </div>
                ...
                <label className="flex flex-col gap-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="0.00"
                      className="w-full h-[46px] pl-8 pr-20 rounded-xl text-[14px] font-bold text-[var(--c-text)] tabular-nums outline-none border border-[var(--c-accent-border-strong)] focus:border-[var(--c-accent-border-strong)] transition"
                      style={{ background: 'var(--c-surface-soft)' }}
                    />
