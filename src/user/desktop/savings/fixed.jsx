...
import {
  ...
  ArrowUpRight, Clock, Calendar, Percent, Coins, ChevronLeft,
  AlertTriangle, Zap,
} from 'lucide-react'
...
function CreateLockModal({ product, onClose, onSubmit, submitting, success }) {
  ...
  const maturityDate = numDays > 0
    ? fmtDate(new Date(Date.now() + numDays * 86400000))
    : null
          ...
          >
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-[9.5px] uppercase tracking-[1.3px] text-brand-accent font-bold m-0">New lock</p>
            ...
            {success ? (
              <motion.div key="success" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center text-center px-6 py-10">
                ...
                <p className="text-[12.5px] text-[var(--c-text-muted)] m-0 mt-1.5 max-w-[280px] leading-relaxed">
                  {fmtN(success.principal)} locked in <span className="font-semibold text-[var(--c-text)]">{success.name}</span> for {success.lockDays} days
                </p>
                <div className="w-full mt-6 rounded-2xl border border-[var(--c-border-soft)] overflow-hidden" style={{ background: 'var(--c-surface-soft)' }}>
                  {[
                    ...
                    { label: 'Amount locked',  value: fmtN(success.principal) },
                ...
                {activeTier && (
                  <label className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      className="h-[46px] px-4 rounded-xl text-[14px] font-bold text-[var(--c-text)] tabular-nums outline-none border border-[var(--c-border)] focus:border-[var(--c-accent-border-strong)] transition"
                      style={{ background: 'var(--c-surface-soft)' }}
                    />
                  </label>
                )}
...
function LockCard({ lock, onWithdraw, withdrawing, penalty }) {
    ...
    >
      ...
      <div className="p-5">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {isMature ? (
                ...
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-[0.8px]" style={{ background: LOCK_BG, color: LOCK, border: `1px solid ${LOCK_BORDER}` }}>
                  <Zap size={8} strokeWidth={3} /> Locked
                </span>
              )}
              {lock.apy > 0 && (
        ...
        <div className="flex items-center justify-between gap-2 mb-4">
          <div>
            <p className="text-[11px] text-[var(--c-text-muted)] m-0">Amount locked</p>
            <p className="text-[22px] font-black text-[var(--c-text)] m-0 tabular-nums tracking-[-0.5px] leading-tight">{fmtN(lock.principal)}</p>
          </div>
          <div className="text-right">
            <p className="text-[11px] text-[var(--c-text-muted)] m-0">Interest earned</p>
            <p className="text-[15px] font-black tabular-nums m-0 leading-tight" style={{ color: LOCK }}>+{fmtN(lock.interestEarned)}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[
            ...
            { label: 'Duration', value: lock.daysTotal != null ? `${lock.daysTotal}d` : '—', icon: Clock },
            { label: isMature ? 'Total payout' : 'Proj. payout', value: fmtN(lock.principal + (isMature ? lock.interestEarned : lock.projectedInterest)), icon: Coins, gold: true },
          ].map(({ label, value, icon: Icon, gold }) => (
            <div key={label} className="flex flex-col gap-0.5 p-3 rounded-xl" style={{ background: 'var(--c-surface-soft)', border: '1px solid var(--c-border-soft)' }}>
        ...
        {!isMature && (
          <>
            <div className="relative w-full h-1.5 rounded-full overflow-hidden mb-1.5" style={{ background: 'var(--c-border-soft)' }}>
              <motion.div
        ...
        <div className="pt-4 border-t border-[var(--c-border-soft)]">
          {confirming ? (
            <div className="flex items-start gap-3 p-3.5 rounded-xl border" style={{ background: 'rgba(251,146,60,0.07)', borderColor: 'rgba(251,146,60,0.25)' }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10.5px] text-[var(--c-text-muted)] m-0 mt-0.5 leading-relaxed">
                  {penalty > 0
                    ? `A ${penalty}% penalty applies. You may receive less than your locked amount.`
            ...
            <button
              type="button"
              onClick={() => isMature ? onWithdraw(lock.id) : setConfirming(true)}
              disabled={withdrawing}
              className={[
            ...
            >
              {withdrawing
                ...
                : <><ArrowUpRight size={12} strokeWidth={2.5} /> {isMature ? 'Withdraw funds' : 'Early withdraw'}</>
              }
            </button>
          )}
        </div>
...
export default function DesktopFixedSavings() {
  ...
  async function handleCreate(payload) {
    ...
    if (r.success) {
      ...
      alert({ type: 'success', title: 'Plan locked', message: `${payload.name} locked for ${payload.lockDays} days at ${payload.apy}% p.a.` })
    } else {
    ...
    <div className="flex flex-col gap-5 max-w-[1240px] mx-auto pb-10">
      ...
      <header className="flex items-end justify-between gap-3 flex-wrap">
        <div>
          ...
          <h1 className="text-[22px] font-black tracking-[-0.5px] text-[var(--c-text)] m-0 mt-1">Your locked plans</h1>
          <p className="text-[12px] text-[var(--c-text-muted)] m-0 mt-0.5">
            Lock funds for a fixed term and earn up to {maxApy > 0 ? `${maxApy}%` : 'a higher rate'} p.a. guaranteed
          </p>
        </div>
        <button
          type="button"
          ...
          style={{ background: 'linear-gradient(135deg,#C9A227,#f0d060)', color: '#0A1F44', border: '1px solid rgba(232,197,71,0.5)', boxShadow: '0 8px 24px -6px rgba(201,162,39,0.5)' }}
        >
          <Plus size={14} strokeWidth={2.6} /> New lock
        </button>
      </header>
      ...
      >
        ...
        <div className="relative grid grid-cols-1 min-[860px]:grid-cols-[1.4fr_1fr] gap-0 divide-y min-[860px]:divide-y-0 min-[860px]:divide-x divide-white/[0.07]">
          <div className="p-5">
            <span className="inline-flex items-center gap-1.5 text-[9.5px] uppercase tracking-[1.5px] font-bold mb-4 block" style={{ color: LOCK }}>
              <Lock size={9} /> Total locked portfolio
            </span>
            <p className="text-[11px] text-white/50 m-0">Total locked</p>
          ...
          <div className="grid grid-cols-2 gap-px p-5">
            {[
              ...
              { label: 'Active locks',    value: String(activeLocks.length),         icon: Lock },
              { label: 'Matured',         value: String(maturedLocks.length),        icon: Check },
              { label: 'Max APY',         value: maxApy > 0 ? `${maxApy}% p.a.` : '—', icon: TrendingUp },
        ...
        <section className="flex flex-col gap-3">
          <div className="flex items-center gap-2">
            <h2 className="inline-flex items-center gap-2 text-[14px] font-black m-0 text-[var(--c-text)] tracking-[-0.2px]">
              <Lock size={14} style={{ color: LOCK }} /> Active locks
            </h2>
            <span className="text-[10.5px] font-semibold text-[var(--c-text-muted)]">{activeLocks.length}</span>
          </div>
          <div className="grid grid-cols-1 min-[860px]:grid-cols-2 gap-4">
            {activeLocks.map(lock => (
              <LockCard key={lock.id} lock={lock} onWithdraw={handleWithdraw} withdrawing={withdrawingId === lock.id} penalty={fixedProduct?.penalty || 0} />
            ))}
          </div>
        </section>
      )}
      ...
      {maturedLocks.length > 0 && (
        <section className="flex flex-col gap-3">
          <div className="flex items-center gap-2">
            <h2 className="inline-flex items-center gap-2 text-[14px] font-black m-0 text-[var(--c-text)] tracking-[-0.2px]">
              <Check size={14} style={{ color: MATURE }} /> Matured plans
            </h2>
            <span className="text-[10.5px] font-semibold text-[var(--c-text-muted)]">{maturedLocks.length}</span>
          </div>
          <div className="grid grid-cols-1 min-[860px]:grid-cols-2 gap-4">
      ...
      {fixedLocks.length === 0 && (
        <div className="flex flex-col items-center text-center py-20 px-4 rounded-[20px] border border-[var(--c-border)]" style={{ background: 'var(--c-surface)' }}>
          ...
          <h3 className="text-[16px] font-black text-[var(--c-text)] m-0 tracking-[-0.2px]">No locked plans</h3>
          <p className="text-[12.5px] text-[var(--c-text-muted)] m-0 mt-1.5 max-w-[320px] leading-relaxed">
            Lock your money for a fixed term to earn up to {maxApy > 0 ? `${maxApy}%` : 'a higher rate'} p.a. — guaranteed.
          </p>
          <button
            type="button"
            onClick={() => { setShowModal(true); setSuccess(null) }}
