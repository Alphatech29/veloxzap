...
import {
  ArrowLeft, Copy, Check, AlertTriangle, Info, Clock, ShieldCheck,
  Sparkles, Download, ReceiptText, Sun, Moon, X,
} from 'lucide-react'
...
const HERO_BG = `radial-gradient(520px 220px at 110% -10%, rgba(201, 162, 39, 0.30), transparent 60%), linear-gradient(135deg, rgba(20, 42, 92, 0.96), rgba(10, 31, 68, 1))`

export default function DesktopCoinInfo() {
  ...
  const variant = selected != null ? variants[selected] : (variants.length === 1 ? variants[0] : (variants.length > 1 ? variants[0] : null))
  const effectiveSelected = selected != null ? selected : (variants.length > 1 ? 0 : null)
      ...
      <header className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <p className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-[1.3px] text-brand-accent font-bold m-0">
            <Sparkles size={10} /> Deposit
          </p>
          <h1 className="text-[20px] font-bold tracking-[-0.4px] text-[var(--c-text)] m-0 mt-1">
            {primary ? `${primary.name} · Deposit` : (symbol || '').toUpperCase()}
          </h1>
        </div>
        <button
          type="button"
          onClick={() => navigate('/user/wallet/history')}
          className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[var(--c-accent-soft)] border border-[var(--c-accent-border)] text-[9.5px] font-bold uppercase tracking-[1px] text-brand-accent hover:border-[var(--c-accent-border-strong)] transition shrink-0"
        >
        ...
        <section className="grid grid-cols-1 min-[960px]:grid-cols-[1.45fr_1fr] gap-4 items-start">
          <div className="flex flex-col gap-4">
            <article className="relative overflow-hidden rounded-2xl border border-[rgba(201,162,39,0.28)] shadow-[0_18px_44px_-14px_rgba(2,7,23,0.5)]">
              {/* Header band */}
              <div className="relative overflow-hidden p-5" style={{ background: HERO_BG }}>
                <span aria-hidden className="pointer-events-none absolute -top-10 -right-10 w-[160px] h-[160px] rounded-full bg-brand-accent/[0.16] blur-2xl" />
                <div className="relative flex items-center justify-between gap-2">
                  <div className="flex items-center gap-3">
                    <img src={(variant || primary).icon} alt={(variant || primary).symbol} className="w-11 h-11 rounded-full border border-white/20" />
                    <div className="leading-tight">
                      <p className="text-[15px] font-bold text-white m-0">{(variant || primary).name}</p>
                  ...
                  {chain && (
                    <span className={[
                      'inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[9.5px] font-bold uppercase tracking-[0.6px]',
              ...
              <div className="bg-[var(--c-surface)]">
                {addressesLoading ? (
                  <div aria-hidden className="m-5 h-48 rounded-xl bg-[var(--c-surface-soft)] animate-pulse" />
                ) : !chain ? (
                  <div className="flex items-start gap-2.5 p-5">
                    <span className="inline-flex items-center justify-center w-8 h-8 rounded-[10px] bg-[var(--c-accent-soft)] text-brand-accent border border-[var(--c-accent-border)] shrink-0">
                      <Info size={14} />
                    </span>
                    <p className="text-[12px] text-[var(--c-text-muted)] m-0 leading-snug">
                      {variant.symbol} deposits aren't supported yet. Don't send {variant.symbol} to any address — it won't be credited.
                    </p>
                  </div>
                ) : !addressRow ? (
                  <div className="flex items-start gap-2.5 p-5">
                    <span className="inline-flex items-center justify-center w-8 h-8 rounded-[10px] bg-[var(--c-warn-bg)] text-[var(--c-warn)] border border-[var(--c-warn)] shrink-0">
                  ...
                  <div className="grid grid-cols-1 min-[640px]:grid-cols-[auto_1fr] gap-5 p-5">
                    <span className="relative inline-flex items-center justify-center rounded-2xl bg-white border border-[var(--c-border)] shadow-[0_10px_24px_-10px_rgba(0,0,0,0.28)] p-4 self-start mx-auto min-[640px]:mx-0">
                      <QRCodeCanvas ref={qrRef} value={addressRow.address} size={160} bgColor="#ffffff" fgColor="#0A1F44" level="H" />
                      <img
                        src={variant.icon}
                        alt=""
                        aria-hidden
                    ...
                    <div className="flex flex-col gap-3 min-w-0">
                      <div className="w-full flex flex-col gap-1.5 rounded-xl bg-[var(--c-surface-soft)] border border-[var(--c-border-soft)] px-4 py-3">
                        ...
                        <div className="flex flex-row justify-between items-start gap-2.5">
                          <span className="w-full text-start text-[13.5px] font-mono font-semibold text-[var(--c-text)] break-all leading-snug">
                            {addressRow.address}
                          </span>
                          <button
                            type="button"
                      ...
                      <div className="flex items-start gap-2.5 p-3 rounded-xl bg-[var(--c-warn-bg)] border border-[var(--c-warn)] border-opacity-30">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          ...
                          className="inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-[12px] font-bold tracking-[0.2px] bg-[var(--c-surface-soft)] border border-[var(--c-border)] text-[var(--c-text)] hover:border-[var(--c-accent-border)] active:scale-[0.98] transition"
                        >
                          Save picture
                        </button>
                        <button
            ...
            {variants.length > 1 && (
              <article className="rounded-xl bg-[var(--c-surface)] border border-[var(--c-border)] overflow-hidden">
                <header className="flex items-center justify-between gap-2 px-4 py-3 border-b border-[var(--c-border)]">
                  <h3 className="text-[12px] font-bold m-0 text-[var(--c-text)]">Network</h3>
                </header>
                  ...
                  {variants.map((v, i) => (
                    <li key={v.asset}>
                      <button
                        ...
                        className={[
                          'w-full grid grid-cols-[auto_1fr_auto] items-center gap-2.5 px-3 py-2.5 rounded-lg text-left transition',
                          i === effectiveSelected
                            ? 'bg-[var(--c-accent-soft)] border border-[var(--c-accent-border)]'
                            : 'hover:bg-[var(--c-surface-soft)] border border-transparent',
                        ].join(' ')}
                      >
                        <img src={v.icon} alt={v.symbol} className="w-8 h-8 rounded-full shrink-0" />
                        ...
                        {i === effectiveSelected && <Check size={13} className="text-brand-accent shrink-0" />}
                      </button>
                    </li>
                  ))}
                </ul>
        ...
        {savePreviewOpen && (
          <>
            <motion.div
              ...
              onClick={() => setSavePreviewOpen(false)}
            />
            <motion.div
            ...
            >
              <div
                role="dialog"
                aria-modal="true"
                aria-label="Save deposit card"
                className="pointer-events-auto w-full max-w-[380px] max-h-[85vh] overflow-y-auto flex flex-col rounded-[20px] shadow-[0_32px_100px_rgba(0,0,0,0.55),0_0_0_1px_rgba(201,162,39,0.15)]"
                style={{ background: 'var(--c-surface)' }}
                onClick={e => e.stopPropagation()}
              >
                <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-[var(--c-border)]">
                  <h2 className="text-[14px] font-bold text-[var(--c-text)] m-0">Save deposit card</h2>
                  <button type="button" onClick={() => setSavePreviewOpen(false)} className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-[var(--c-surface-soft)] border border-[var(--c-border)] text-[var(--c-text-muted)] hover:text-[var(--c-text)] active:scale-90 transition shrink-0">
                    <X size={14} />
                  </button>
                </div>
